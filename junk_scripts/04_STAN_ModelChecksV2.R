...
# looping through the different canopy levels, looking at growth at 
# different levels
for(Lvl in 1:n_Lvls){
  ...
  fit1 <- All_STAN_fits.L[[paste0("growthCont_CL", Lvl)]]
  ...
  ppc4 <- ppc_stat_grouped(log(growth_CL.df$growth2),
                           g_reps, stat = kurtosis,
                           group = growth_CL.df$Mnemonic) +
    labs(title = paste0(site,"_growCL",Lvl,"_PPC_kurt_bySpp"))
